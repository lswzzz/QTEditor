...
void AddSprite::setStartPos(Point pos)
{
	startPos = pos;
	drawRect = false;
}

void AddSprite::setMovedPos(Point pos)
{
	movedPos = pos;
	drawRect = true;
}

void AddSprite::setEndedPos(Point pos)
{
	endedPos = pos;
	drawRect = false;
}

void AddSprite::draw(Renderer *renderer, const Mat4 &transform, uint32_t flags)
{
	_customCommand.init(_globalZOrder);
	_customCommand.func = CC_CALLBACK_0(AddSprite::onDraw, this, transform, flags);
	renderer->addCommand(&_customCommand);
}

void AddSprite::onDraw(const Mat4 &transform, uint32_t flags)
{
	if (drawRect){
...
std::vector<Point> AddSprite::getRange(int gridW, int gridH)
{
