...
void DeleteSprite::setStartPos(Point pos)
{
	startPos = pos;
	drawRect = false;
}

void DeleteSprite::setMovedPos(Point pos)
{
	movedPos = pos;
	drawRect = true;
}

void DeleteSprite::setEndedPos(Point pos)
{
	endedPos = pos;
	drawRect = false;
}

void DeleteSprite::draw(Renderer *renderer, const Mat4 &transform, uint32_t flags)
{
	_customCommand.init(_globalZOrder);
	_customCommand.func = CC_CALLBACK_0(DeleteSprite::onDraw, this, transform, flags);
	renderer->addCommand(&_customCommand);
}

void DeleteSprite::onDraw(const Mat4 &transform, uint32_t flags)
{
	if (drawRect){
		...
		glLineWidth(1.0f);
		DrawPrimitives::setDrawColor4B(220, 203, 188, 255);
...
std::vector<Point> DeleteSprite::getRange(int gridW, int gridH)
{
...
void DeleteSprite::setType(int type)
{
	m_type = type;
}

int DeleteSprite::getType()
{
	return m_type;
}